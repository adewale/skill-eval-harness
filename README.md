...
- `tool_faults` on an answer-population case: hand-authored tool results (`[{"tool", "match"?, "output", "is_error"?, "times"?}]`) that the `run-subagent` tool replay store serves to **every arm and repetition** in place of the live or recorded result, so a permission denial, malformed result, or ambiguous output is the controlled stimulus and only the skill differs. Only meaningful when the skill says something about that failure; otherwise both arms show the model's own recovery and the case reads as no-lift. A fault `output` is model-visible text: describe the failure, never the recovery the assertions expect. `is_error: true` declares the result an error result, and a conservation law then requires the run's trace to show it as one (see `docs/commands.md`). Trigger cases cannot declare faults; Jetty export refuses them; a native CLI runner refuses them unless told `--skip-fault-cases`, in which case `run-subagent --only-fault-cases` runs them into the same runs directory under the same answer design. The bundled demo's `c-fault-denied-runner` case is the worked example (`examples/demo-skill/README.md`).
...
| `skill-benchmark run-subagent` | In-process backend seam: any provider via `--agent-cmd`, a harness-mediated `--tool-bridge`, tool replay and `tool_faults` (`--only-fault-cases`), multi-turn `turns`. |
