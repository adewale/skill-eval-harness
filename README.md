...
- `tool_faults` on an answer-population case: hand-authored tool results (`[{"tool", "match"?, "output", "times"?}]`) that the `run-subagent` tool replay store serves to **every arm and repetition** in place of the live or recorded result, so a permission denial, malformed result, or ambiguous output is the controlled stimulus and only the skill differs. Only meaningful when the skill says something about that failure; otherwise both arms show the model's own recovery and the case reads as no-lift. A fault `output` is model-visible text: describe the failure, never the recovery the assertions expect. Native CLI runners and Jetty export refuse these rows; trigger cases cannot declare them.
