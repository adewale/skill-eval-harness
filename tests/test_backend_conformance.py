...
class IsolationConformanceTests(unittest.TestCase):
    ...
    def test_unattended_approval_is_paired_with_a_constraint(self):
        # Auto-approval is unavoidable headless, so it must not be granted
        # bare: codex pairs it with `--sandbox read-only`, vibe with a tool
        # allowlist, agy with `--sandbox`.
        #
        # For agy the pairing is necessary but NOT sufficient, and this test
        # does not claim otherwise: `--sandbox` covers terminal operations only,
        # and `--dangerously-skip-permissions` auto-approves the sandbox-bypass
        # prompt too (antigravity-cli#36), both reproduced on 1.1.8. What is
        # asserted here is that the constraint the adapter declares is really
        # passed -- so the flag cannot be dropped silently, and the pairing
        # becomes real containment if upstream closes #36. The exposure that
        # remains is recorded in every run's environment.json.
        for backend, profile in sorted(PROFILES.items()):
            with self.subTest(backend=backend):
                command = run_backend(backend, profile.healthy_stdout).environment.get("command") or ""
                if any(token in command for token in profile.unattended_tokens):
                    self.assertTrue(
                        any(token in command for token in profile.constraint_tokens),
                        f"{backend}: unattended approval with no sandbox or tool allowlist: {command}")
