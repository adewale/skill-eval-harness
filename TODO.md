...
- [ ] **Exapt a reusable detector library from real usage — revisit end of 2026.** Do not design the detector set up front. Once the harness has a usage trail, mine the GitHub fork graph and the issue tracker at the end of 2026 and *exapt* — repurpose from observed use, not design — the deterministic checks forkers actually hand-rolled and the gaps issues actually report, so the library is grounded in real usage rather than guesses. A detector earns its place when usage shows two or more independent skills re-implementing it, the bar `anti-slop-writing`, `swiss-poster-skill`, and `guardrails-skill` already meet by hand. Feeds the judge-presets / named-detector-library work above; each harvested detector ships with the fire/pass meta-fixtures from the confidence-floor section of [`docs/eval-framework-roadmap-spec.md`](docs/eval-framework-roadmap-spec.md) before it is trusted.
