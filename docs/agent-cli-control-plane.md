...
| Final answer channel | JSON result envelope (`result`) | `--output-last-message <file>`; JSONL is trace, not final-answer source | final assistant segment in a complete `stream-json` lifecycle; JSON envelope `response` for judges | final assistant `LLMMessage.content` from `--output json` / `--output streaming` |
...
| Config isolation | temp workspace; `--no-session-persistence`; optional portable config copy; `--bare` is a future stricter option when API-key auth is available | isolated `CODEX_HOME` outside the model workdir; copy only auth/config files; `--ignore-user-config --ignore-rules`; `--ephemeral` | isolated `GEMINI_CLI_HOME` outside the model workdir; environment auth or credential files plus selected auth type only; reject project provider controls | isolated `VIBE_HOME` outside the model workdir; copy only `.env` when `MISTRAL_API_KEY` is absent; never copy user skills/config |
