...
| Judge path | native backend or `--judge-cmd` | Native backends use provider-specific schema/final-answer channels where available; `--judge-cmd` remains the universal stdin→stdout JSON escape hatch. |
...
| Config isolation | temp workspace; `--no-session-persistence`; optional portable config copy; `--bare` is a future stricter option when API-key auth is available | isolated `CODEX_HOME`; copy only auth/config files; `--ignore-user-config --ignore-rules`; `--ephemeral` | isolated `VIBE_HOME`; copy only `.env` when `MISTRAL_API_KEY` is absent; never copy user skills/config |
| Skill discovery for trigger evals | project `.claude/skills`; primary evidence is `Skill` tool use, path evidence fallback | `$CODEX_HOME/skills`; path evidence from JSONL commands/messages | project `.agents/skills`; primary evidence is native `skill` tool call, path evidence fallback |
