...
- `variant_instruction` (`skill_benchmark.py:588`) / `task_variants` (`:631`) / `prepared_task_rows` (`:676`) — `skill_paths` is currently the manifest's, identical for every variant.
- `safe_task_json` (`skill_benchmark.py:1845`) / `build_jetty_payload` (`:1887`) — Jetty export; ablation reads `task["skill_paths"]`, uploads flat files via `JettyClient.upload` with a basename-only `remote_path_hint`.
...
`validate_manifest` (`skill_benchmark.py:468`), per ablation:
