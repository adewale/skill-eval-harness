...
- `validate_manifest` ablation block (`skill_benchmark.py:515`) — today checks only non-empty `id` + `removed_component`.
- `variant_instruction` (`skill_benchmark.py:648`) / `task_variants` (`:691`) / `prepared_task_rows` (`:736`) — `skill_paths` is currently the manifest's, identical for every variant.
- `safe_task_json` (`skill_benchmark.py:1920`) / `build_jetty_payload` (`:1962`) — Jetty export; ablation reads `task["skill_paths"]`, uploads flat files via `JettyClient.upload` with a basename-only `remote_path_hint`.
...
`validate_manifest` (`skill_benchmark.py:515`), per ablation:
