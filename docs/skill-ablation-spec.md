...
- `validate_manifest` ablation block (`skill_benchmark.py:510`) — today checks only non-empty `id` + `removed_component`.
- `variant_instruction` (`skill_benchmark.py:643`) / `task_variants` (`:686`) / `prepared_task_rows` (`:731`) — `skill_paths` is currently the manifest's, identical for every variant.
- `safe_task_json` (`skill_benchmark.py:1906`) / `build_jetty_payload` (`:1948`) — Jetty export; ablation reads `task["skill_paths"]`, uploads flat files via `JettyClient.upload` with a basename-only `remote_path_hint`.
...
`validate_manifest` (`skill_benchmark.py:510`), per ablation:
