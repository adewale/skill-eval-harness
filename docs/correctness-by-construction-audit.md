...
- Packaging inventory, `ty` source coverage, and the versioned trigger-semantic module inventory
  ...
  deliberately conservative at module granularity: it names every module split out of the former
  `skill_benchmark.py` monolith, so any edit to that code still invalidates trigger evidence, and
  every packaged module must be classified as identified or audited non-trigger.
...
- the modules split out of `skill_benchmark.py` are all in the conservative trigger identity, so it
  still invalidates on unrelated edits to them until it is narrowed to what trigger paths import; and
