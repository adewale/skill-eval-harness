...
- `skill-benchmark judge --judge-backend gemini` uses `--output-format json`, parses only the envelope's `response` as verdict JSON, and preserves the raw envelope plus provider metadata in judge transcripts.
- Every invocation uses a fresh `GEMINI_CLI_HOME` outside the model workdir. Environment auth is preferred; otherwise only Gemini's credential files and `security.auth.selectedType` are copied. User skills, extensions, MCP configuration, hooks, context, policies, history, and sessions are not copied.
- A user-tier TOML policy denies every tool, then answer runs allow only `glob`, `grep_search`, `list_directory`, `read_file`, and `read_many_files`; judges keep the deny-all rule. The harness also requests sandboxing and records that administrator policy can override user-tier policy.
- Workspace `.gemini` controls and `GEMINI.md` files are rejected before invocation so an eval fixture cannot silently replace the harness control plane. Usage is normalized from `stats` when present; absent usage and unsupported dollar cost remain explicit `missing` rather than zero.
...
Offline tests mirror the official Gemini CLI stream/JSON conformance shapes. The opt-in token-backed answer smoke is `RUN_GEMINI_SMOKE=1`; it is not part of default CI.
