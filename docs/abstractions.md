...
ships Pi answer smoke (`examples/adewale-workspace/run_pi_smoke.py`), Codex (`run_codex:10642`), Claude (`run_claude:10829`, capturing real
...
subagent runner (`run_subagent:13709`, which hosts record/replay tool I/O via `ToolReplayStore`),
Jetty (`JettyClient:4164` and the export/run/import commands), and any runner that writes the
...
(`prompt_assertion_leakage_findings:815`), and the split discipline are the part of the tool
