...
ships Pi answer smoke (`examples/adewale-workspace/run_pi_smoke.py`), Codex (`run_codex:10785`), Claude (`run_claude:10996`, capturing real
...
subagent runner (`run_subagent:13599`, which hosts record/replay tool I/O via `ToolReplayStore`),
Jetty (`JettyClient:4082` and the export/run/import commands), and any runner that writes the
