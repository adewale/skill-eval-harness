...
(`run_pi_trigger_eval.py`), Codex (`run_codex:3508`), Claude (`run_claude:3653`, capturing real
per-run cost), the in-process subagent runner (`run_subagent:4441`, which hosts record/replay
tool I/O via `ToolReplayStore`), Jetty (`JettyClient:2131` and the export/run/import commands),
