...
(`run_pi_trigger_eval.py`), Codex (`run_codex:3602`), Claude (`run_claude:3750`, capturing real
per-run cost), the in-process subagent runner (`run_subagent:4767`, which hosts record/replay
tool I/O via `ToolReplayStore`), Jetty (`JettyClient:2195` and the export/run/import commands),
...
(`prompt_assertion_leakage_findings:356`), and the split discipline are the part of the tool
