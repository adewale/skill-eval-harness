...
ships Pi answer smoke (`examples/adewale-workspace/run_pi_smoke.py`), Codex (`run_codex:10579`), Claude (`run_claude:10765`, capturing real
...
subagent runner (`run_subagent:13425`, which hosts record/replay tool I/O via `ToolReplayStore`),
Jetty (`JettyClient:4125` and the export/run/import commands), and any runner that writes the
