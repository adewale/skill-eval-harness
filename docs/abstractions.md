...
ships Pi answer smoke (`examples/adewale-workspace/run_pi_smoke.py`), Codex (`run_codex:5229`), Claude (`run_claude:5374`, capturing real
per-run cost), Mistral Vibe (`run-agent --agent vibe`, using isolated `VIBE_HOME` outside the workdir), the in-process
subagent runner (`run_subagent:6789`, which hosts record/replay tool I/O via `ToolReplayStore`),
Jetty (`JettyClient:2284` and the export/run/import commands), and any runner that writes the
...
(`prompt_assertion_leakage_findings:412`), and the split discipline are the part of the tool
