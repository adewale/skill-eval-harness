...
**Trigger comparison** — `skill-benchmark trigger-compare`, the trigger population's paired causal gate: a baseline trigger-matrix report against an `--ablation` report of the same canonical revision. Complete agent/model cells are reported but collapsed to one delta per authored query and polarity before the sign-flip test, so agents and models are repeated measurements rather than independent inference units. `causal_confirmation` requires matching ablation IDs, verified provenance, coverage, a negative aggregate mean, and significance to produce `confirmed_causal`; a significant change in the improving direction is not a regression. The comparison upgrades trigger evidence from a single-arm `raw_measurement` to `confirmed_causal` / `refuted` / `indeterminate`.
