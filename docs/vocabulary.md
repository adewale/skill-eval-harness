...
**Spend ceiling** — a runtime USD cap (`--max-cost-usd`) on every paid loop: native answer runners, the subagent seam, Jetty, and judges. Each loop plans one frozen `SpendLedger` (`spend_contracts.py`), asks it `can_start` before every run, and charges each completed run's cost measurement into it; once the ceiling is reached no further run *starts* (runs in flight finish and are paid for), and the command exits `2`. A run whose cost cannot be charged is the ledger's terminal `UnpricedRun`: the spent total becomes a partial known subtotal and the loop stops. Unstarted runs are listed in `spend-ceiling.json` beside `answer-design.json`, so the benchmark's existing availability rules mark the design incomplete and the report's `answer_design.stopped_by` says why. A backend that cannot report dollars must be given `--assumed-cost-per-run-usd` or the ceiling refuses to start; an unobservable cost mid-suite stops the loop (`cost_unobservable`) rather than counting as free. `suite-run`'s `--max-estimated-cost-usd` is the projected, pre-run half of the same policy.
