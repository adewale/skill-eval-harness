...
| agy case erroring with `second terminal update for a step already completed` | Two `DONE` records for one `step_index`. One lifecycle is one tool call, so accepting both would double every counter the step feeds | Check whether the release now re-reports a completed step; if so, the lifecycle handling in `agy_contracts.py` needs to absorb it before the run can be scored |
| agy case erroring with `names conversation X in a stream established as Y` | The trace holds records from more than one conversation, so it does not describe one observation | Trace files are per-run — check for a concatenated or replayed trace; a real single-process capture cannot produce this |
| An agy trace showing a `command`/`tool_call` event with `status: in_progress` | A tool step that started and never reported `DONE`. It is published so a `command_not_ran`-style assertion can see it, and counted by nothing | Nothing to fix in the harness — the run itself left a tool unfinished, which the trigger observation also reports as unavailable rather than as a clean negative |
