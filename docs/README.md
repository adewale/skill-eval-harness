...
| Did my skill change HOW the model works, not just whether it passes? | [`did-my-skill-change-how-the-model-works.md`](did-my-skill-change-how-the-model-works.md) |
| Did removing this description actually break discovery? | [`did-removing-this-break-discovery.md`](did-removing-this-break-discovery.md) |
...
supports but the docs don't walk"; every entry there is ticked, and an unticked
entry would mark a journey whose machinery has shipped but whose walkthrough has
not. When a new journey earns a slot, its entry names the machinery that already
