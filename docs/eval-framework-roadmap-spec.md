...
(`skill_benchmark.py:11481`) and the fan-out in `prepared_task_rows` (`:1609`).
...
`without_skill` on the same case (`build_paired_summary` (`:15732`)). That number is believable only
...
- **Abstractions used or changed:** none in the engine. Adds
  ...
  `assertion_result` (`:11481`) directly. That function has no direct unit test today, and its
...
- **Abstractions used or changed:** none. A test-time guard around `grade_case_variant` (`:14263`).
...
and negative-delta flags (`build_benchmark_report` (`:17295`)) are computed right — sits a layer
...
- **Abstractions used or changed:** `tool_call` and `structured_output` are deterministic, so
  ...
  `assertion_result`. `tool_call` reuses `command_events` (`:6523`) and the `command_order`
  logic; `structured_output` extends `json_field_equals` with JSON-Schema validation.
  `factuality` adds no core code: it is a named rubric that `judge_prompt` (`:12064`) renders
...
- **Abstractions used or changed:** a new type in `TEXT_ASSERTIONS` (`:261`), implemented in
  `assertion_result` (`:11481`). It reads `output.md` (or a named artifact), applies an optional
...
- **Abstractions used or changed:** extend `run_script_assertion` (`:11349`) and
  `assertion_result` (`:11481`) to optionally parse a score from the oracle's stdout — a JSON line
...
- **Abstractions used or changed:** `prepared_task_rows` (`:1609`) gains a `model` dimension
  ...
  so grading needs no change. `build_benchmark_report` (`:17295`) groups `by_variant` within
  `by_model`, and `build_paired_summary` (`:15732`) computes lift per (case, model).
...
- **Abstractions used or changed:**
  - `assertion_result` (`:11481`) gains an optional `score` (0-1 or a normalized 1-5) and
  ...
  - **Anchored `graded_dimensions`** as a `judge` assertion shape:
    ...
    (`:12064`) renders the dimensions; the result carries per-dimension scores in `evidence`.
  ...
  - `grade_case_variant` (`:14263`) splits totals into critical, gated, and soft. A `critical`
  ...
  - **Statistical lift** in `build_paired_summary` (`:15732`): alongside the raw delta, compute a
...
- **Abstractions used or changed:** this lives in the runner, not core grading. Recording sits
  beside `write_trace_artifacts` (`:8395`): a `tool-replay.json` keyed per tool, with
...
- **Abstractions used or changed:** `normalize_trace_record` (`:7453`) and
  `normalize_trace_records` (`:8111`) keep their inputs but emit OTel GenAI semantic-key
...
- **Abstractions used or changed:** mostly a discipline made first-class, not new machinery.
  `prepared_task_rows` (`:1609`) already omits `review_rubric` from generation payloads unless
...
- **Abstractions changed (core contract):** a case gains an optional `turns` list. The
  ...
  `read_output_base` (`:6370`) and `discover_run_bases` learn the turn layout; runners drive the
...
- **Slice-lift concentration (from the macro-evals notebook):** compute, per slice, where lift (or
  ...
  up from per-case lift. `build_slice_summary` (`:15958`) already groups by domain/difficulty/
