...
(`eval_grading.py:487`) and the fan-out in `prepared_task_rows` (`prepared_tasks.py:235`).
...
`without_skill` on the same case (`build_paired_summary` (`skill_benchmark.py:1799`)). That number is believable only
...
- **Abstractions used or changed:** none in the engine. Adds
  ...
  `assertion_result` (`eval_grading.py:487`) directly. That function has no direct unit test today, and its
...
- **Abstractions used or changed:** none. A test-time guard around `grade_case_variant` (`eval_grading.py:811`).
...
and negative-delta flags (`build_benchmark_report` (`skill_benchmark.py:3324`)) are computed right — sits a layer
...
- **Abstractions used or changed:** `tool_call` and `structured_output` are deterministic, so
  ...
  `factuality` adds no core code: it is a named rubric that `judge_prompt` (`judge_tasks.py:278`) renders
...
- **Abstractions used or changed:** read an optional `judge` block in the manifest; add a check
  in `audit_manifest_report` (`skill_benchmark.py:6114`) comparing the declared judge model against `jetty.model`
...
- **Follow-on:** surface the guide's rules where they are checkable. Extend the messaging in
  `prompt_assertion_leakage_findings` (`eval_manifests.py:449`) and `fixture_recommendations` (`skill_benchmark.py:5807`) to point
...
- **Abstractions used or changed:** a new type in `TEXT_ASSERTIONS` (`eval_manifests.py:44`), implemented in
  `assertion_result` (`eval_grading.py:487`). It reads `output.md` (or a named artifact), applies an optional
...
- **Abstractions used or changed:** an optional `oracle` tier on an assertion
  ...
  `script` is `demo` unless marked, judge/live are `live`). `build_benchmark_report` (`skill_benchmark.py:3324`)
  reports, per case, the share of its pass rate carried by `strong` oracles;
  `audit_manifest_report` (`skill_benchmark.py:6114`) warns when a case passes only on weak ones. This extends
...
- **Abstractions used or changed:** extend `run_script_assertion` (`eval_grading.py:355`) and
  `assertion_result` (`eval_grading.py:487`) to optionally parse a score from the oracle's stdout — a JSON line
...
- **Abstractions used or changed:** reads the cross-run history from 2.6. A `prune` report (or a
  flag in `build_benchmark_report` (`skill_benchmark.py:3324`)) marks a case a removal candidate when, across the
  last N runs, it never failed and never showed lift (`with_skill` == `without_skill` every time).
  `audit_manifest_report` (`skill_benchmark.py:6114`) lists the candidates; removal stays a human decision.
...
- **Abstractions used or changed:** `prepared_task_rows` (`prepared_tasks.py:235`) gains a `model` dimension
  ...
  so grading needs no change. `build_benchmark_report` (`skill_benchmark.py:3324`) groups `by_variant` within
  `by_model`, and `build_paired_summary` (`skill_benchmark.py:1799`) computes lift per (case, model).
...
- **Abstractions used or changed:**
  - `assertion_result` (`eval_grading.py:487`) gains an optional `score` (0-1 or a normalized 1-5) and
  ...
  - **Anchored `graded_dimensions`** as a `judge` assertion shape:
    ...
    (`judge_tasks.py:278`) renders the dimensions; the result carries per-dimension scores in `evidence`.
  ...
  - `grade_case_variant` (`eval_grading.py:811`) splits totals into critical, gated, and soft. A `critical`
  ...
  - **Statistical lift** in `build_paired_summary` (`skill_benchmark.py:1799`): alongside the raw delta, compute a
...
- **Abstractions used or changed:** a consumer of `build_benchmark_report`. Add an append-only
  ...
  the `compare_results` (`skill_benchmark.py:4491`) logic.
...
- **Abstractions used or changed:** extend `render_viewer` (`skill_benchmark.py:5288`) with a `serve` mode and
...
- **Slice-lift concentration (from the macro-evals notebook):** compute, per slice, where lift (or
  ...
  up from per-case lift. `build_slice_summary` (`skill_benchmark.py:2025`) already groups by domain/difficulty/
