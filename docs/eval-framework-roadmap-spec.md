...
- **Abstractions used or changed:** read an optional `judge` block in the manifest; add a check
  in `audit_manifest_report` (`:20198`) comparing the declared judge model against `jetty.model`
...
- **Follow-on:** surface the guide's rules where they are checkable. Extend the messaging in
  `prompt_assertion_leakage_findings` (`:813`) and `fixture_recommendations` (`:19891`) to point
...
- **Abstractions used or changed:** an optional `oracle` tier on an assertion
  ...
  `audit_manifest_report` (`:20198`) warns when a case passes only on weak ones. This extends
...
- **Abstractions used or changed:** reads the cross-run history from 2.6. A `prune` report (or a
  ...
  `audit_manifest_report` (`:20198`) lists the candidates; removal stays a human decision.
...
- **Abstractions used or changed:** extend `render_viewer` (`:19354`) with a `serve` mode and
