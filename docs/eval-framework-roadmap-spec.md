...
(`skill_benchmark.py:5097`) and the fan-out in `prepared_task_rows` (`:769`).
...
`without_skill` on the same case (`build_paired_summary` (`:7108`)). That number is believable only
...
- **Abstractions used or changed:** none in the engine. Adds
  ...
  `assertion_result` (`:5097`) directly. That function has no direct unit test today, and its
...
- **Abstractions used or changed:** none. A test-time guard around `grade_case_variant` (`:6463`).
...
and negative-delta flags (`build_benchmark_report` (`:8031`)) are computed right — sits a layer
...
- **Abstractions used or changed:** `tool_call` and `structured_output` are deterministic, so
  ...
  `factuality` adds no core code: it is a named rubric that `judge_prompt` (`:5379`) renders
...
- **Abstractions used or changed:** read an optional `judge` block in the manifest; add a check
  in `audit_manifest_report` (`:9961`) comparing the declared judge model against `jetty.model`
...
- **Follow-on:** surface the guide's rules where they are checkable. Extend the messaging in
  `prompt_assertion_leakage_findings` (`:390`) and `fixture_recommendations` (`:9687`) to point
...
- **Abstractions used or changed:** a new type in `TEXT_ASSERTIONS` (`:94`), implemented in
  `assertion_result` (`:5097`). It reads `output.md` (or a named artifact), applies an optional
...
- **Abstractions used or changed:** an optional `oracle` tier on an assertion
  ...
  `script` is `demo` unless marked, judge/live are `live`). `build_benchmark_report` (`:8031`)
  reports, per case, the share of its pass rate carried by `strong` oracles;
  `audit_manifest_report` (`:9961`) warns when a case passes only on weak ones. This extends
...
- **Abstractions used or changed:** extend `run_script_assertion` (`:4994`) and
  `assertion_result` (`:5097`) to optionally parse a score from the oracle's stdout — a JSON line
...
- **Abstractions used or changed:** reads the cross-run history from 2.6. A `prune` report (or a
  flag in `build_benchmark_report` (`:8031`)) marks a case a removal candidate when, across the
  last N runs, it never failed and never showed lift (`with_skill` == `without_skill` every time).
  `audit_manifest_report` (`:9961`) lists the candidates; removal stays a human decision.
...
- **Abstractions used or changed:** `prepared_task_rows` (`:769`) gains a `model` dimension
  ...
  so grading needs no change. `build_benchmark_report` (`:8031`) groups `by_variant` within
  `by_model`, and `build_paired_summary` (`:7108`) computes lift per (case, model).
...
- **Abstractions used or changed:**
  - `assertion_result` (`:5097`) gains an optional `score` (0-1 or a normalized 1-5) and
  ...
  - **Anchored `graded_dimensions`** as a `judge` assertion shape:
    ...
    (`:5379`) renders the dimensions; the result carries per-dimension scores in `evidence`.
  ...
  - `grade_case_variant` (`:6463`) splits totals into critical, gated, and soft. A `critical`
  ...
  - **Statistical lift** in `build_paired_summary` (`:7108`): alongside the raw delta, compute a
...
- **Abstractions used or changed:** a consumer of `build_benchmark_report`. Add an append-only
  ...
  the `compare_results` (`:8500`) logic.
...
- **Abstractions used or changed:** extend `render_viewer` (`:9207`) with a `serve` mode and
...
- **Slice-lift concentration (from the macro-evals notebook):** compute, per slice, where lift (or
  ...
  up from per-case lift. `build_slice_summary` (`:7241`) already groups by domain/difficulty/
