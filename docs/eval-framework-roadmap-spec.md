...
(`skill_benchmark.py:5843`) and the fan-out in `prepared_task_rows` (`:811`).
...
`without_skill` on the same case (`build_paired_summary` (`:8353`)). That number is believable only
...
- **Abstractions used or changed:** none in the engine. Adds
  ...
  `assertion_result` (`:5843`) directly. That function has no direct unit test today, and its
  `contains_any`, `excludes_any`, and `not_regex` branches go unexercised even though their types
  are declared in the assertion registries (`TEXT_ASSERTIONS:113`).
- **Design:** one table-driven test loads every pair and asserts the detector fires on `should-fire`
  ...
  lint (`prompt_assertion_leakage_findings` (`:413`)). Every detector bug then becomes a permanent
...
- **Testing:** the fixtures are the test. A meta-test asserts every name in `OBJECTIVE_ASSERTIONS`
  (`:141`) has a fixture pair, so a new detector cannot land unverified.
...
- **Abstractions used or changed:** none. A test-time guard around `grade_case_variant` (`:7394`).
...
and negative-delta flags (`build_benchmark_report` (`:9363`)) are computed right — sits a layer
...
- **Abstractions used or changed:** `tool_call` and `structured_output` are deterministic, so
  ...
  `assertion_result`. `tool_call` reuses `command_events` (`:3011`) and the `command_order`
  logic; `structured_output` extends `json_field_equals` with JSON-Schema validation.
  `factuality` adds no core code: it is a named rubric that `judge_prompt` (`:6187`) renders
...
- **Abstractions used or changed:** read an optional `judge` block in the manifest; add a check
  in `audit_manifest_report` (`:11304`) comparing the declared judge model against `jetty.model`
...
- **Follow-on:** surface the guide's rules where they are checkable. Extend the messaging in
  `prompt_assertion_leakage_findings` (`:413`) and `fixture_recommendations` (`:11030`) to point
...
- **Abstractions used or changed:** a new type in `TEXT_ASSERTIONS` (`:113`), implemented in
  `assertion_result` (`:5843`). It reads `output.md` (or a named artifact), applies an optional
...
- **Abstractions used or changed:** an optional `oracle` tier on an assertion
  ...
  `script` is `demo` unless marked, judge/live are `live`). `build_benchmark_report` (`:9363`)
  reports, per case, the share of its pass rate carried by `strong` oracles;
  `audit_manifest_report` (`:11304`) warns when a case passes only on weak ones. This extends
...
- **Abstractions used or changed:** extend `run_script_assertion` (`:5733`) and
  `assertion_result` (`:5843`) to optionally parse a score from the oracle's stdout — a JSON line
...
- **Abstractions used or changed:** reads the cross-run history from 2.6. A `prune` report (or a
  flag in `build_benchmark_report` (`:9363`)) marks a case a removal candidate when, across the
  last N runs, it never failed and never showed lift (`with_skill` == `without_skill` every time).
  `audit_manifest_report` (`:11304`) lists the candidates; removal stays a human decision.
...
- **Abstractions used or changed:** `prepared_task_rows` (`:811`) gains a `model` dimension
  ...
  so grading needs no change. `build_benchmark_report` (`:9363`) groups `by_variant` within
  `by_model`, and `build_paired_summary` (`:8353`) computes lift per (case, model).
...
- **Abstractions used or changed:**
  - `assertion_result` (`:5843`) gains an optional `score` (0-1 or a normalized 1-5) and
  ...
  - **Anchored `graded_dimensions`** as a `judge` assertion shape:
    ...
    (`:6187`) renders the dimensions; the result carries per-dimension scores in `evidence`.
  ...
  - `grade_case_variant` (`:7394`) splits totals into critical, gated, and soft. A `critical`
  ...
  - **Statistical lift** in `build_paired_summary` (`:8353`): alongside the raw delta, compute a
...
- **Abstractions used or changed:** this lives in the runner, not core grading. Recording sits
  beside `write_trace_artifacts` (`:4372`): a `tool-replay.json` keyed per tool, with
...
- **Abstractions used or changed:** `normalize_trace_record` (`:3797`) and
  `normalize_trace_records` (`:4104`) keep their inputs but emit OTel GenAI semantic-key
...
- **Abstractions used or changed:** a new optional manifest construct (`datasets`, plus a case
  `template` referencing a dataset id). `iter_cases` (`:312`) materializes template by row into
...
- **Abstractions used or changed:** a consumer of `build_benchmark_report`. Add an append-only
  ...
  the `compare_results` (`:9835`) logic.
...
- **Abstractions used or changed:** mostly a discipline made first-class, not new machinery.
  `prepared_task_rows` (`:811`) already omits `review_rubric` from generation payloads unless
  `--include-answer-key`; extend `validate_manifest` (`:588`) to require that a `holdout`/
...
- **Abstractions used or changed:** extend `render_viewer` (`:10550`) with a `serve` mode and
...
- **Abstractions changed (core contract):** a case gains an optional `turns` list. The
  ...
  `read_output_base` (`:2857`) and `discover_run_bases` learn the turn layout; runners drive the
...
- **Slice-lift concentration (from the macro-evals notebook):** compute, per slice, where lift (or
  ...
  up from per-case lift. `build_slice_summary` (`:8486`) already groups by domain/difficulty/
...
- `version` on the manifest (`validate_manifest:588`) becomes meaningful: `validate` accepts both
