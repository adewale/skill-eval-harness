...
`without_skill` on the same case (`build_paired_summary` (`:5547`)). That number is believable only
...
- **Abstractions used or changed:** none. A test-time guard around `grade_case_variant` (`:4943`).
...
and negative-delta flags (`build_benchmark_report` (`:6190`)) are computed right — sits a layer
...
- **Abstractions used or changed:** read an optional `judge` block in the manifest; add a check
  in `audit_manifest_report` (`:7828`) comparing the declared judge model against `jetty.model`
...
- **Follow-on:** surface the guide's rules where they are checkable. Extend the messaging in
  `prompt_assertion_leakage_findings` (`:317`) and `fixture_recommendations` (`:7678`) to point
...
- **Abstractions used or changed:** an optional `oracle` tier on an assertion
  ...
  `script` is `demo` unless marked, judge/live are `live`). `build_benchmark_report` (`:6190`)
  reports, per case, the share of its pass rate carried by `strong` oracles;
  `audit_manifest_report` (`:7828`) warns when a case passes only on weak ones. This extends
...
- **Abstractions used or changed:** reads the cross-run history from 2.6. A `prune` report (or a
  flag in `build_benchmark_report` (`:6190`)) marks a case a removal candidate when, across the
  last N runs, it never failed and never showed lift (`with_skill` == `without_skill` every time).
  `audit_manifest_report` (`:7828`) lists the candidates; removal stays a human decision.
...
- **Abstractions used or changed:** `prepared_task_rows` (`:682`) gains a `model` dimension
  ...
  so grading needs no change. `build_benchmark_report` (`:6190`) groups `by_variant` within
  `by_model`, and `build_paired_summary` (`:5547`) computes lift per (case, model).
...
- **Abstractions used or changed:**
  ...
  - `grade_case_variant` (`:4943`) splits totals into critical, gated, and soft. A `critical`
  ...
  - **Statistical lift** in `build_paired_summary` (`:5547`): alongside the raw delta, compute a
...
- **Abstractions used or changed:** a consumer of `build_benchmark_report`. Add an append-only
  ...
  the `compare_results` (`:6648`) logic.
...
- **Abstractions used or changed:** extend `render_viewer` (`:7317`) with a `serve` mode and
...
- **Slice-lift concentration (from the macro-evals notebook):** compute, per slice, where lift (or
  ...
  up from per-case lift. `build_slice_summary` (`:5682`) already groups by domain/difficulty/
