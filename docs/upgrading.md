...
These changes merged after the 0.6.0 tag. This section becomes the next release's boundary when
it is tagged; 0.6.0 users follow it before installing from `main`.
...
`skill-benchmark` console script fails with `ModuleNotFoundError` outside the repository root.
